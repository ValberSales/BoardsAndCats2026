...
const TopMenu: React.FC = () => {
  ...
  const renderNavLinks = (isSidebar: boolean = false) => (
    <>
        ...
        {isAdmin && (
            <NavLink to="/admin/dashboard" className={({ isActive }) => `nav-link-item ${isActive ? 'active' : ''}`} onClick={() => isSidebar && setVisibleSidebar(false)}>
                <i className="pi pi-shield text-2xl nav-icon-svg mr-2 flex align-items-center" style={{ fontSize: '24px' }}></i>
