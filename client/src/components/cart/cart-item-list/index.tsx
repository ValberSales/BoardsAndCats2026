...
import { getEffectivePrice, getDiscountPercentage } from "@/types/product";
                ...
                <div className="cart-item-container p-3">
                    <div className="cart-item-inner">
                        ...
                        <div className="cart-image-wrapper">
                            ...
                            {item.product.promo && (
                                <Tag 
                                    value={`${getDiscountPercentage(item.product)}% OFF`} 
                                    severity="success" 
                                    className="absolute font-bold" 
                                    style={{ top: "4px", left: "4px", fontSize: "0.65rem", zIndex: 1 }} 
                                />
                            )}
                        </div>
                        ...
                        <div className="cart-item-details">
                            ...
                            <div className="cart-item-footer">
                                <div className="flex flex-column gap-1">
                                    {item.product.promo && item.product.discountType && item.product.discountValue !== undefined ? (
                                        <>
                                            <span className="text-xs line-through m-0" style={{ color: '#888888' }}>
                                                {(item.product.price * item.quantity).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                                            </span>
                                            <span className="cart-item-price text-primary font-bold">
                                                {(getEffectivePrice(item.product) * item.quantity).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                                            </span>
                                        </>
                                    ) : (
